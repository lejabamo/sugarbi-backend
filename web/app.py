...
@app.route('/api/dashboard/filtrado')
def get_dashboard_filtrado():
    """Obtener datos del dashboard con filtros dinámicos"""
    try:
        engine = get_db_connection()
        
        # Obtener parámetros de filtro (acepta 'año' o 'anio')
        anio = request.args.get('año', type=int) or request.args.get('anio', type=int)
        mes = request.args.get('mes', type=int)
        zona = request.args.get('zona')
        variedad = request.args.get('variedad', type=int)
        finca = request.args.get('finca', type=int)
        ...
        if anio is not None:
            where_conditions.append("t.anio = :anio")
            params['anio'] = anio
            
        if mes is not None:
            where_conditions.append("t.mes = :mes")
            params['mes'] = mes
            
        if zona:
            where_conditions.append("z.codigo_zona = :zona")
            params['zona'] = zona
            
        if variedad is not None:
            where_conditions.append("v.variedad_id = :variedad")
            params['variedad'] = variedad
            
        if finca is not None:
            where_conditions.append("f.finca_id = :finca")
            params['finca'] = finca
        
        where_clause = "WHERE " + " AND ".join(where_conditions) if where_conditions else ""
        
        # Query principal para estadísticas (sqlalchemy.text con parámetros ASCII)
        stats_query = text(f"""
        SELECT 
            COUNT(DISTINCT f.finca_id) as total_fincas,
            COUNT(DISTINCT v.variedad_id) as total_variedades,
            COUNT(DISTINCT z.codigo_zona) as total_zonas,
            COUNT(*) as total_cosechas,
            ...
            AVG(h.rendimiento_teorico) as promedio_rendimiento
        FROM hechos_cosecha h
        ...
        stats_result = pd.read_sql(stats_query, engine, params=params)
        stats = stats_result.iloc[0].to_dict()
        
        # Query para top cosechas
        top_cosechas_query = text(f"""
        SELECT 
            f.nombre_finca,
            v.nombre_variedad,
            z.nombre_zona,
        ...
        top_cosechas_result = pd.read_sql(top_cosechas_query, engine, params=params)
        top_cosechas = top_cosechas_result.to_dict('records')
        
        return jsonify({
            "success": True,
            "data": {
                "estadisticas": stats,
                "top_cosechas": top_cosechas
            }
        })
        
    except Exception as e:
        print(f"❌ Error en get_dashboard_filtrado: {e}")
        return jsonify({"success": False, "error": str(e)}), 500

@app.route('/api/filtros/cascada')
def get_filtros_cascada():
    """Obtener opciones de filtros en cascada (dinámicos)"""
    try:
        engine = get_db_connection()
        
        # Obtener filtros aplicados (acepta 'año' o 'anio')
        anio = request.args.get('año', type=int) or request.args.get('anio', type=int)
        mes = request.args.get('mes', type=int)
        zona = request.args.get('zona')
        variedad = request.args.get('variedad', type=int)
        
        # Construir condiciones WHERE para filtros aplicados
        where_conditions = []
        params = {}
        ...
        FROM hechos_cosecha h
        JOIN dimtiempo t ON h.codigo_tiempo = t.tiempo_id
        {where_clause.replace('t.anio = :anio', '1=1') if anio is not None else where_clause}
        ORDER BY t.anio DESC
        """)
        años_result = pd.read_sql(años_query, engine, params=params if anio is None else {k: v for k, v in params.items() if k != 'anio'})
        años = [int(row['anio']) for _, row in años_result.iterrows()]
        
        # Obtener meses disponibles (si hay año seleccionado)
        meses = []
        if anio is not None:
            meses_query = text(f"""
            SELECT DISTINCT t.mes, t.nombre_mes
            FROM hechos_cosecha h
            JOIN dimtiempo t ON h.codigo_tiempo = t.tiempo_id
            {where_clause.replace('t.mes = :mes', '1=1') if mes is not None else where_clause}
            ORDER BY t.mes ASC
            """)
            meses_result = pd.read_sql(meses_query, engine, params=params if mes is None else {k: v for k, v in params.items() if k != 'mes'})
            meses = [{'mes': int(row['mes']), 'nombre': str(row['nombre_mes'])} for _, row in meses_result.iterrows()]
        
        # Obtener zonas disponibles
        zonas_query = text(f"""
        SELECT DISTINCT z.codigo_zona, z.nombre_zona
        FROM hechos_cosecha h
        JOIN dimzona z ON h.codigo_zona = z.codigo_zona
        {where_clause.replace('z.codigo_zona = :zona', '1=1') if zona else where_clause}
        ORDER BY z.codigo_zona
        """)
        zonas_result = pd.read_sql(zonas_query, engine, params=params if not zona else {k: v for k, v in params.items() if k != 'zona'})
        zonas = [{'codigo': str(row['codigo_zona']), 'nombre': str(row['nombre_zona'])} for _, row in zonas_result.iterrows()]
        
        # Obtener variedades disponibles
        variedades_query = text(f"""
        SELECT DISTINCT v.variedad_id, v.nombre_variedad
        FROM hechos_cosecha h
        JOIN dimvariedad v ON h.codigo_variedad = v.variedad_id
        {where_clause.replace('v.variedad_id = :variedad', '1=1') if variedad is not None else where_clause}
        ORDER BY v.nombre_variedad
        """)
        variedades_result = pd.read_sql(variedades_query, engine, params=params if variedad is None else {k: v for k, v in params.items() if k != 'variedad'})
        variedades = [{'id': int(row['variedad_id']), 'nombre': str(row['nombre_variedad'])} for _, row in variedades_result.iterrows()]
        
        # Obtener fincas disponibles
        fincas_query = text(f"""
        SELECT DISTINCT f.finca_id, f.codigo_finca, f.nombre_finca
        FROM hechos_cosecha h
        JOIN dimfinca f ON h.id_finca = f.finca_id
        {where_clause}
        ORDER BY f.nombre_finca
        """)
        fincas_result = pd.read_sql(fincas_query, engine, params=params)
        fincas = [{'id': int(row['finca_id']), 'codigo': str(row['codigo_finca']), 'nombre': str(row['nombre_finca'])} for _, row in fincas_result.iterrows()]
        ...
        print(f"❌ Error en get_filtros_cascada: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
